...
\usepackage{float}
\usepackage{tikz}
\usetikzlibrary{shapes, arrows, positioning, calc, shapes.geometric, backgrounds, fit}
...
\midrule
\textbf{Baza danych} & PostgreSQL 17 \\
\midrule
...
% Warstwa użytkownika
\node[rectangle, draw, fill=blue!20, rounded corners, minimum width=3cm, minimum height=1cm] (browser) {Przeglądarka};
\node[above=0.3cm of browser, font=\footnotesize] {Użytkownik};

% NGINX jako reverse proxy
\node[rectangle, draw, fill=orange!20, rounded corners, minimum width=2.5cm, minimum height=1cm, below=of browser] (nginx) {NGINX};
\node[right=0.2cm of nginx, font=\footnotesize, align=left] {Reverse Proxy\\(port 443)};

% Kontenery Docker
\node[rectangle, draw, fill=green!20, rounded corners, minimum width=2.5cm, minimum height=1cm, below left=1.5cm and 0.5cm of nginx] (frontend) {Frontend};
\node[below=0.1cm of frontend, font=\footnotesize] {SvelteKit};

\node[rectangle, draw, fill=purple!20, rounded corners, minimum width=2.5cm, minimum height=1cm, below right=1.5cm and 0.5cm of nginx] (backend) {Backend};
\node[below=0.1cm of backend, font=\footnotesize] {FastAPI};

% Baza i storage
\node[rectangle, draw, fill=blue!10, rounded corners, minimum width=2cm, minimum height=0.8cm, below=2cm of backend, xshift=-1.5cm] (db) {PostgreSQL};
\node[rectangle, draw, fill=yellow!20, rounded corners, minimum width=2cm, minimum height=0.8cm, below=2cm of backend, xshift=1.5cm] (minio) {MinIO};

% Strzałki
\draw[<->, thick] (browser) -- node[right, font=\footnotesize] {HTTPS} (nginx);
\draw[->, thick] (nginx) -- node[left, font=\footnotesize, align=right] {/* (HTML/JS)} (frontend);
\draw[->, thick] (nginx) -- node[right, font=\footnotesize, align=left] {/api/*} (backend);
\draw[<->, thick] (backend) -- (db);
\draw[<->, thick] (backend) -- (minio);

% Ramka Docker
\begin{scope}[on background layer]
\node[rectangle, draw=gray, dashed, rounded corners, fit=(nginx)(frontend)(backend)(db)(minio), inner sep=0.5cm, label={[font=\footnotesize]above right:Docker Compose}] {};
\end{scope}
\end{tikzpicture}
...
\begin{enumerate}
    \item Użytkownik otwiera przeglądarkę i wchodzi na adres aplikacji (HTTPS)
    \item NGINX (reverse proxy) kieruje żądania ścieżek \texttt{/*} do kontenera Frontend
    \item Frontend (SvelteKit) zwraca statyczne pliki HTML, CSS i JavaScript do przeglądarki
    \item Kod JavaScript wykonywany w przeglądarce wysyła żądania API (\texttt{/api/*}) przez NGINX
    \item NGINX przekazuje żądania \texttt{/api/*} do kontenera Backend (FastAPI)
    \item Backend komunikuje się z PostgreSQL (metadane, użytkownicy) oraz MinIO (pliki)
\end{enumerate}

\textbf{Uwaga:} Frontend i Backend nie komunikują się bezpośrednio między sobą. Cała komunikacja z API odbywa się z poziomu przeglądarki użytkownika przez NGINX.
...
\begin{itemize}
    ...
    \item komunikacja z API po HTTPS przez NGINX (żądania \texttt{/api/*})
\end{itemize}
